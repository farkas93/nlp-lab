...
def _validate_dpo_columns(dataset: Dataset, *, split_name: str) -> None:
    columns = set(dataset.column_names)
    required = {"prompt", "chosen", "rejected"}
    missing = sorted(list(required - columns))
    if missing:
        raise ValueError(
            f"Missing required DPO columns in split={split_name}: {missing}. "
            "Expected at least prompt/chosen/rejected."
        )


def load_dpo_manifest_dataset(
    *,
    manifest_uri: str,
    train_split: str,
    eval_split: str,
    ...
    _validate_dpo_columns(train_dataset, split_name=train_split)
    _validate_dpo_columns(eval_dataset, split_name=eval_split)
